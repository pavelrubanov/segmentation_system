"""Тесты core/leaf_pipeline.py: чистка маски, PCA-кроп, дедупликация, auto_segment."""
...
def _mask(h, w, *blobs):
    """h×w маска с прямоугольниками-блобами (y0, y1, x0, x1)."""
...
class TestPreprocessMask:
    ...
    def test_keeps_largest_component(self):
        m = _mask(50, 50, (5, 25, 5, 25), (40, 43, 40, 43))   # большая + крошка
...
class TestPcaCrop:
    ...
    def test_apex_ends_up_at_top(self):
        # Треугольник: широкое основание сверху, узкий апекс снизу.
        # pca_crop должен перевернуть так, чтобы апекс оказался сверху.
        h, w = 100, 80
        m = np.zeros((h, w), dtype=np.uint8)
        for y in range(h):
        ...
        # После переворота масса должна быть внизу (= апекс наверху).
        assert out_mask[mid:].sum() > out_mask[:mid].sum()

    def test_too_few_pixels_falls_back_to_axis_bbox(self):
        # < 8 пикселей → fallback _axis_bbox без warpAffine.
...
class TestDedupeAndTransformLeaf:
    ...
    def test_low_iou_kept(self):
        ...
        m2 = _mask(100, 100, (40, 80, 40, 80))   # перекрытие маленькое
        assert len(dedupe_and_transform_leaf(img, [m1, m2], iou_threshold=0.9)) == 2

    def test_high_iou_keeps_larger(self):
        img = np.zeros((100, 100, 3), dtype=np.uint8)
        big = _mask(100, 100, (10, 90, 10, 90))     # 80×80
        small = _mask(100, 100, (12, 88, 12, 88))   # 76×76, IoU > 0.9
        out = dedupe_and_transform_leaf(img, [small, big], iou_threshold=0.9)
        assert len(out) == 1
        kept_mask = out[0][0]
        # Оставшаяся маска ближе по площади к big
...
class TestTransformLeaf:
    def test_combined_preprocess_and_pca(self):
        # Лист с дырой и носом + наклонная ориентация
        img = np.zeros((150, 150, 3), dtype=np.uint8)
        img[40:120, 30:80] = 200   # «лист»
        img[70:80, 50:60] = 0      # дыра внутри листа
...
class TestAutoSegment:
    def test_upscales_to_full_resolution(self):
        ...
        # predictor.predict_all вызовется на small (200×200, DOWNSCALE=2)
